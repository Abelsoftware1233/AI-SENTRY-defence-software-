...
(() => {
  ...
  /* ==========================================================
     ...
     rule-based scanners). Dit is GEEN AI-model — het is
  ...
  $('rl-flood-btn').addEventListener('click', () => {
    ...
    pushFeed('badge-bot', 'FLOOD', `Flood-simulatie: 20 verzoeken → ${allowed} toegestaan, ${blocked} geblokkeerd`, '');
    if (blocked > 0) {
      setTrust(-15, 'Flood-patroon gedetecteerd in verzoeken');
    }
  });

  /* ------------------------ init ------------------------ */

  pushFeed('badge-info', 'INIT', 'SENTRY console geladen — detectiemodules actief', '');
  runFingerprintCheck();

  // Als na 8 seconden nog geen enkele muisbeweging is geregistreerd,
  // is dat zelf al een signaal (scrapers voeren nooit mousemove uit).
  setTimeout(() => {
    if (state.mouseSamples.length === 0) {
      $('mouse-verdict').textContent = 'geen muisactiviteit gedetecteerd';
      $('mouse-verdict').style.color = 'var(--warn)';
      $('mouse-state').textContent = 'NO DATA';
      setTrust(-10, 'Geen muisbeweging gedetecteerd binnen 8 seconden');
      pushFeed('badge-suspect', 'CHECK', 'Geen enkele mousemove geregistreerd na 8s', '');
    }
  }, 8000);
